...
class MultiDataset(Dataset):
	def __init__(self, root_dir, train, augment = False, dataset=None, list_ids=None, list_labels=None, val_size=80, fold=0, test = False, missing_data = False, inference=False, missing_p=0):
		...
		if self.missing == True:
			if self.inference == False:
				...
				if mask == 0:
					...
					indexes = random.sample(range(8),self.missing_p) 
					for i in indexes:
						var_bl[i]=0
					indexes = random.sample(range(8),self.missing_p) 
					for i in indexes:
...
class ClinNet(nn.Module):
	...
	def forward(self,left_mri, right_mri, left_cl, right_cl, dem):
...
class MultiNet(nn.Module):
	...
	def forward(self, l_mri, r_mri, l_cl, r_cl, dem):
		cl, _ = self.clin_module(l_mri, r_mri, l_cl, r_cl, dem)
...
class MultiAdaptNet(nn.Module):
	...
	def forward(self, l_mri, r_mri, l_cl, r_cl, dem):
		cl, outcl = self.clin_module(l_mri, r_mri, l_cl, r_cl, dem)
...
for fold in range(nb_folds):
	...
	datatest = MultiDataset("/data1/cecilia/ADNI_mri", train = False, augment=False, dataset = test_dataset, list_ids = test_ids, list_labels = test_labels, val_size=val_size, test= True, missing_data=True, missing_p = missing_p, fold=fold, inference=True)
	...
	if (model_name == "multi_adapt_net") or (model_name == "multi_adapt_manq_net"):
		tdsnet = MultiAdaptNet()
	if (model_name == "multi_net") or (model_name == "multi_manq_net"):
		tdsnet = MultiNet()
	if (model_name == "clin_net") or (model_name == "clin_manq_net"):
		tdsnet = ClinNet()
	tdsnet.load_state_dict(torch.load(model_name+"_"+str(fold)+".pt"))
	...
	with torch.no_grad():
		...
		for k, data in enumerate(test_dataloader):
			...
			dem = dem.to(device)
			if (model_name == "clin_net") or (model_name == "clin_manq_net"):
				y_pred = tdsnet(imgs, imgs2, var_bl, var_m12, dem)[1].cpu().detach().numpy()
			else:
